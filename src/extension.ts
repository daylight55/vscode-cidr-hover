...
export function activate(context: vscode.ExtensionContext) {
    ...
    const hoverProvider = vscode.languages.registerHoverProvider('*', {
        provideHover(document: vscode.TextDocument, position: vscode.Position) {
            ...
            content.appendMarkdown(`**Host address**: \`${cidrInfo.hostAddress}\`\n\n`);
            content.appendMarkdown(`**Addresses in network**: \`${cidrInfo.totalIPs.toLocaleString()}\`\n\n`);
            content.appendMarkdown(`**Network range**: \`${cidrInfo.startIP} - ${cidrInfo.endIP}\`\n\n`);
            content.appendMarkdown(`**Usable range**: \`${cidrInfo.firstUsableIP} - ${cidrInfo.lastUsableIP}\``);
